...
public class HistoricoPedidosAfvDAO {
	...
	private static Integer qdeDiasExport = 10;
	private NotificaUsuarioDAO notificaUsuarioDAO;
	private NotificaUsuario notificaUser;
	private Long codGrupo = 4l; // TI
	private Long codUsurEmetente = 0l;
	private Long importancia = 3l;
	private boolean logAtivo = true;
	...
	public void deletaCabecalhoHistoricoPedAFV() throws Exception {
		...
		try {
			sql.executeUpdate();
			log(true, "Cabeçalhos dos pedidos deletados sem erro.");
	...
	public void deletaItensCabecalhoHistoricoPedAFV() throws Exception {
		...
		try {
			sql.executeUpdate();
			log(true, "Itens dos pedidos deletados sem erro.");
		} catch (IOException | SQLException e) {
	...
	public void insereCabecHistoricoPedidosAFV() throws Exception {
		...
		try {
			sql.executeUpdate();
			log(logAtivo, "Cabeçalhos dos pedidos inseridos sem erro.");
		} catch (IOException | SQLException e) {
	...
	public void insereCabecHistoricoPedidosRefaturadosAFV() throws Exception {
		...
		try {
			sql.executeUpdate();
			log(logAtivo, "Cabeçalhos dos pedidos refaturados inseridos sem erro.");
		} catch (IOException | SQLException e) {
...
//		sql.appendSql("       AND ITEPROVIDER.CODIGOPRODUTO IS NULL   ");
		...
		sql.appendSql("INSERT INTO AFV_ANDROID.TE_HISTPEDIDOITEM@PROVIDER ");
		sql.appendSql("SELECT NUMPEDIDOEMP, ");
		sql.appendSql("       CODIGOPRODUTO, ");
		sql.appendSql("       DESCRICAOPRODUTO, ");
		sql.appendSql("       QTDEVENDA, ");
		sql.appendSql("       QTDEBONIFICADA, ");
		sql.appendSql("       QTDECORTADA, ");
		sql.appendSql("       DESCONTO, ");
		sql.appendSql("       VALORVENDA, ");
		sql.appendSql("       CODIGOCONDPAGTO, ");
		sql.appendSql("       CODIGOEMPRESAESP, ");
		sql.appendSql("       CODIGOEMPRESA, ");
		sql.appendSql("       DATAATUALIZACAO, ");
		sql.appendSql("       MOTIVOCORTE, ");
		sql.appendSql("       VALORDESCONTO, ");
		sql.appendSql("       TOTALPESO, ");
		sql.appendSql("       VRTOTALCOMISSAO, ");
		sql.appendSql("       SALDOQTDEVENDA, ");
		sql.appendSql("       FLAG_GERARVERBA ");
		sql.appendSql("FROM   (SELECT HST.NUMPEDIDOEMP                                   AS NumPedidoEmp, ");
		sql.appendSql("               ITE.CODPROD                                        AS CodigoProduto, ");
		sql.appendSql("               PRO.DESCRPROD                                      AS DescricaoProduto, ");
		sql.appendSql("               ITE.QTDNEG                                         AS QtdeVenda, ");
		sql.appendSql("               NULL                                               AS QtdeBonificada, ");
		...
		sql.appendSql("               NULL                                               AS SaldoQtdeVenda, ");
		sql.appendSql("               NULL                                               AS FLAG_GERARVERBA, ");
		sql.appendSql("               ROW_NUMBER () ");
		sql.appendSql("                 OVER ( ");
		sql.appendSql("                   PARTITION BY HST.NUMPEDIDOEMP, ITE.CODPROD ");
		sql.appendSql("                   ORDER BY ITE.QTDNEG, ITE.CODVOL)               SEQ ");
		sql.appendSql("        FROM   AFV_ANDROID.TE_HISTPEDIDO@PROVIDER HST ");
		sql.appendSql("               INNER JOIN SANKHYA.TGFCAB CAB ");
		sql.appendSql("                       ON CAB.AD_PEDPALM = HST.NUMPEDIDOAFV ");
		sql.appendSql("                          AND CAB.TIPMOV = 'P' ");
		...
		sql.appendSql("               AND ITEPROVIDER.NUMPEDIDOEMP IS NULL ");
		sql.appendSql("               AND ITEPROVIDER.CODIGOPRODUTO IS NULL) ");
		sql.appendSql("WHERE  SEQ = 1  ");
		

		try {
			sql.executeUpdate();
			log(logAtivo, "Itens dos pedidos inseridos sem erro.");
		} catch (IOException | SQLException e) {

			log(true, "Itens dos pedidos: "+  e.getMessage());
			// throw e;
		}
	...
	public void insereItemHistoricoPedidosRefaturadosAFV() throws Exception {
		...
		try {
			sql.executeUpdate();
			log(logAtivo, "Itens dos pedidos refaturados inseridos sem erro.");
		} catch (IOException | SQLException e) {

			log(true, "Itens dos pedidos refaturados: " +  e.getMessage());
			// throw e;
		}
	...
	public void insereHistoricoCabecPedidosDuplicPorRegiaoEcom() throws Exception {
		...
		try {
			...
			log(logAtivo, "Cabeçalhos dos pedidos duplicados inseridos sem erro.");
	...
	public void insereHistoricoItemPedidosDuplicPorRegiaoEcom() throws Exception {
		...
		try {
			...
			log(logAtivo, "Itens dos pedidos duplicados inseridos sem erro.");

		} catch (IOException | SQLException e) {

			log(true,"Itens dos pedidos duplicados: " + e.getMessage());
	...
	private void log(boolean ativo, String descricao) {

		if (ativo) {
			notificaUsuarioDAO = new NotificaUsuarioDAO();
			notificaUser = new NotificaUsuario();

			notificaUser.setCodgrupo(codGrupo);
			notificaUser.setCodusuremetente(codUsurEmetente);
			notificaUser.setDescricao(descricao);
			notificaUser.setDhcriacao(null);
			notificaUser.setIdentificador("PERSONALIZADO");
			notificaUser.setImportancia(importancia);
			notificaUser.setTipo("P");
			notificaUser.setTitulo("Exportação Hist. Pedidos AFV");
			try {
